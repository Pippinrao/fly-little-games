...
{
    // A bundled row takes its trusted bilingual titles from the shared manifest,
    // so nothing is hardcoded for it here; only indexed titles are resolved.
    if (!row.builtin && title.match_kind != 0) {
