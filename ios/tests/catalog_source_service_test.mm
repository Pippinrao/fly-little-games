...
int main(int argc, char **argv) { @autoreleasepool { try {
    ...
    NSURL *copyA = [root URLByAppendingPathComponent:@"copy-a.nes"];
    NSURL *copyB = [root URLByAppendingPathComponent:@"copy-b.nes"];
    [rom writeToURL:copyA atomically:YES]; [rom writeToURL:copyB atomically:YES];
    NSString *copyAUUID = [service addURL:copyA directory:NO error:&error];
    NSString *copyBUUID = [service addURL:copyB directory:NO error:&error];
    check(copyAUUID && copyBUUID, "same ROM imports from two independent sources");
    rows = bridge.catalogSnapshotGames;
    check(rows.count == 2, "raw snapshot retains both variants of the canonical game");
    NSString *duplicateID = rows[0][@"canonicalId"];
    NSString *missingUUID = rows[0][@"sourceUUID"];
    NSURL *missingURL = [missingUUID isEqual:copyAUUID] ? copyA : copyB;
    [fm removeItemAtURL:missingURL error:nil];
    check([[service romDataForCanonicalID:duplicateID error:&error] isEqual:rom], "unreadable preferred fresh copy falls back to another fresh copy");
    check(![service rescanUUID:missingUUID error:&error], "deleted duplicate source becomes stale");
    check([[service romDataForCanonicalID:duplicateID error:&error] isEqual:rom], "remaining fresh duplicate launches instead of stale first variant");
    NSArray *library = [bridge gameCenterFilteredGamesForCategory:@"ALL" query:@""];
    check(library.count == 1 && [library[0][@"freshness"] unsignedIntValue] == 1, "library aggregates duplicate canonical game using fresh variant");
    check([service removeUUID:copyAUUID error:&error] && [service removeUUID:copyBUUID error:&error], "duplicate sources removed");
