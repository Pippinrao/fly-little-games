...
public final class DisplayQualitySettingsTest {
    ...
    @Test public void lockedExtremeRemainsFocusableAndExplainsWhy() {
        try (ActivityScenario<SettingsActivity> scenario =
                     ActivityScenario.launch(SettingsActivity.class)) {
            waitForInteractiveLandscape(scenario, R.id.video_preset_extreme_card);
            onView(withId(R.id.video_preset_extreme_card)).perform(scrollTo())
                    .check(matches(isDisplayed()));
            scenario.onActivity(activity -> {
                View card = activity.findViewById(R.id.video_preset_extreme_card);
                assertTrue(card.isFocusable());
                assertTrue(card.isEnabled());
                assertTrue(card.isClickable());
                assertTrue(card.performClick());
            });
            waitForDialogWindow(scenario);
            onView(withText(R.string.video_extreme_locked_title)).inRoot(isDialog())
                    .check(matches(isDisplayed()));
            onView(withText(R.string.video_extreme_locked_message)).inRoot(isDialog())
                    .check(matches(isDisplayed()));
        }
    }

    private static void waitForInteractiveLandscape(ActivityScenario<SettingsActivity> scenario,
            int targetId) {
        final boolean[] ready = {false};
        for (int attempt = 0; attempt < 80 && !ready[0]; attempt++) {
            scenario.onActivity(activity -> {
                View target = activity.findViewById(targetId);
                ready[0] = activity.getResources().getConfiguration().orientation
        ...
        assertTrue("settings did not reach an interactive landscape state", ready[0]);
    }

    private static void waitForDialogWindow(ActivityScenario<SettingsActivity> scenario) {
        final boolean[] dialogHasFocus = {false};
        for (int attempt = 0; attempt < 40 && !dialogHasFocus[0]; attempt++) {
            scenario.onActivity(activity -> dialogHasFocus[0] = !activity.hasWindowFocus());
            if (!dialogHasFocus[0]) SystemClock.sleep(50L);
        }
        assertTrue("locked Extreme explanation did not take window focus", dialogHasFocus[0]);
    }
}
