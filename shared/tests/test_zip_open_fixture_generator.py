#!/usr/bin/env python3
"""Black-box tests for deterministic ZIP-open fixture generation and checking."""
...
class ZipOpenFixtureGeneratorTest(unittest.TestCase):
    def setUp(self) -> None:
    ...
    def test_check_reports_missing_file_without_recreating_it(self) -> None:
        ...
        missing = corpus / "valid_stored_metadata.zip"
        missing.unlink()
        result = self.run_generator("--check")
    ...
    def test_check_reports_changed_file_without_overwriting_it(self) -> None:
        corpus = self.generate()
        changed = corpus / "valid_stored_metadata.zip"
    ...
    def test_generation_refuses_to_leave_an_orphan(self) -> None:
        corpus = self.generate()
        orphan = corpus / "orphan.zip"
        orphan.write_bytes(b"orphan")
        result = self.run_generator()
