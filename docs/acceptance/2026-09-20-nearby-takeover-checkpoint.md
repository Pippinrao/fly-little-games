...
**当前滚动状态（21:36，UTC+8）：主线仅按附近双人联机计算。QUIC provider A2 关闭资源归属已提交为 `f5649ce`，但 A2 验收矩阵尚有覆盖缺口；engine 退休/关闭账本、Android adapter 回调交接、三端生产 owner/UI/唯一 DUAL 运行器及双 App 试玩仍未完成。10:14 及更早的单机/页面回归仅为历史证据，不能计入联机可用进度。下文各旧时刻均按历史快照阅读，以末尾 21:36 节为最新状态。**
