...
|---|---|---|---|
| `nearby.stage.permission` | 权限 | `nearby.stage.permission.reason` | **Yes** — platform permission query |
...
- The pairing page always renders the stage list from §2.1 in order, with the current (first failing) stage marked and the remaining stages unmarked. Stage *selection* is read from the session ABI once it exists; until then the page marks `nearby.stage.permission` and annotates the rest as `nearby.blocked.session_read`.
...
| 配对 | Anonymous join request | `nearby.join.request_anonymous` heading + `nearby.blocked.session_read`; accept/reject disabled with reason |
...
| 游戏中状态 | every §22.4 field | row label + `nearby.blocked.session_read`; the overlay is reachable only from the existing pause drawer |
