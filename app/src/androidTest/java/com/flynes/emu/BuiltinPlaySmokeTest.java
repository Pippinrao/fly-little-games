...
/**
 * Short single-player play coverage, not completion or device-performance certification.
...
public final class BuiltinPlaySmokeTest {
    private static final long TIMEOUT_MS = 30_000;
    private static final String[] NAVIGATION_KEYS = {"category", "query", "selected", "multiplayerOnly"};
    private final Instrumentation instrumentation = InstrumentationRegistry.getInstrumentation();

    @Test public void everyManifestGamePlaysPausesResumesAndReturnsToLibraryWithoutNearby()
            throws Throwable {
        ...
        File evidence = new File(app.getExternalFilesDir(null),
                "out/evidence/builtin-play-" + System.currentTimeMillis());
    ...
    private void playOne(FlyNesApplication app, BuiltinGames.Entry game, File evidence,
        ...
        press(player, core, GamepadHitMap.Control.START, GamepadView.START);
        press(player, core, GamepadHitMap.Control.A, GamepadView.A);
        press(player, core, GamepadHitMap.Control.RIGHT, GamepadView.RIGHT);
        await("frames continue after gameplay input", () -> frames.sequence() >= started + 60);
        long afterInput = frames.sequence();
        screenshot(new File(evidence, game.canonicalId + "-playing.png"));
        ...
        assertSolo(app, player);
        screenshot(new File(evidence, game.canonicalId + "-paused.png"));
        ...
        screenshot(new File(evidence, game.canonicalId + "-library.png"));
    ...
    private void press(MainActivity player, NesCore core, GamepadHitMap.Control control, int bits) {
        ...
        try {
            instrumentation.runOnMainSync(() -> {
                ...
                boolean drag = control == GamepadHitMap.Control.RIGHT
                ...
                float endX = drag ? x + map.joystickTravelRadius() : x;
                gesture.set(new float[]{endX, y});
                ...
                if (drag) dispatch(pad, endX, y, downTime, MotionEvent.ACTION_MOVE);
            });
