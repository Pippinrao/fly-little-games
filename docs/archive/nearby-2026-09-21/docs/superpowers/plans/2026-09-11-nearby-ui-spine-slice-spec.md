...
- **Initial tab of 好友/附近设备 (decided after A1b-1 raised it, applies to all three platforms).** Open on **附近设备** when no friend is saved, and on **好友** when at least one friend is saved. Do not persist the last-used tab. Rationale: a first-time user has no friends, so opening on 好友 shows only an empty list and hides the page's single actionable path (寻找设备 / 扫描房主二维码); UX rule `content-priority` says show the core content on mobile first, and `empty-states` requires an empty state to offer an action. Because no friend storage exists yet, every build today opens on 附近设备 — which is the honest, useful landing state. When the friend store lands, the same one-line rule switches to 好友 for returning users.
