...
final class ControlLayoutEditorView extends View {
    ...
    @Override protected void onDraw(Canvas c){super.onDraw(c);drawTestFrame(c);if(getWidth()==0)return;GamepadHitMap m=map();if(settings.directionControlMode()==DirectionControlMode.JOYSTICK)drawJoystick(c,m);else drawDpad(c,m);draw(c,m.target(GamepadHitMap.Control.B),"B",ControlLayoutV2.Element.B);draw(c,m.target(GamepadHitMap.Control.A),"A",ControlLayoutV2.Element.A);draw(c,m.target(GamepadHitMap.Control.SELECT),"SELECT",ControlLayoutV2.Element.SELECT);draw(c,m.target(GamepadHitMap.Control.START),"START",ControlLayoutV2.Element.START);}
